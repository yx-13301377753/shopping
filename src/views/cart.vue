<template>
  <div>
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="editCart('jian',item)">-</a>
    ...
    <modal :mdShow="mdShowCart" @close="closes">
<!--      v-on:close="closes"    -->
      <p slot="message" style="font-size: 18px; color: orange; ">确认要删除此商品吗？</p>
      <div slot="btnGroup">
        <a class="btn btn-m" @click="mdShowCart = false">取消</a>
...
<script>
  ...
  export default{
    ...
    // filters:{
    //     c : currency //指定过滤器为c 指定执行含数currency
    // },
    computed:{
      //计算总价
      totalPrice () {
      ...
      checkedCount () {//计算是否全选，返回值是选中的数量
        var i = 0;
        this.cartList.forEach(item=>{
          if(item.checked == '1') i++ //记录购物车中有多少选中的商品
        })
        return i
      },
      checkAllFlag () {
        return this.checkedCount == this.cartList.length//购物车的个数==你选中的个数==全选--> 返回值是true 或者false
      }
    },
    methods : {
      checkOut(){
