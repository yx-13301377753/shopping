...
<!--------------------------删除按钮------------------------->
                    <div class="addr-opration addr-del">
                      <a href="javascript:;" class="addr-del-btn" @click="delAddressConfim(item.addressId)">
...
<!-- 模态框     -->
      <modal :mdShow="isShow" @close="closes">
        <!--      v-on:close="closes"    -->
        <p slot="message" style="font-size: 18px; color: orange; ">确认要删除此地址吗？</p>
        <div slot="btnGroup">
      ...
      <modal :mdShow="isAdd" @close="closes" class="addAddress">
        <!--      v-on:close="closes"    -->
        <p slot="message" style="font-size: 18px; color: orange; ">
          <label>请输入您要输入的地址</label>
        ...
        <div slot="btnGroup">
          ...
          <a class="btn btn-m" @click="addAddress()">确定</a>
        </div>
      </modal>
    <!--   删除了footer   -->
...
<style scoped>
  .addAddress input{ margin-top: 10px; display: block; margin-left: 15px; height: 35px; width: 400px;}
</style>
<script>
      ...
      methods : {
        ...
        addAddress(){
          axios.post('/users/addAddress',{
            "addressId" : new Date() + parseInt(Math.random()*1000),
            "userName" : this.name,
