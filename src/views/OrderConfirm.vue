...
<script>
  ...
  export default{
    ...
    methods : {
      payMent(){
          //获取address路由标签传递的id
        var addressId = this.$route.query.addressId;//获取路由标签的方法this.$route.query.后面跟着是路由的传递的名
        //console.log(addressId);//拿到地址栏传过来的id
      ...
      init(){
        axios.get("/users/cartList").then(res=>{
          ...
          this.cartList.forEach(item=>{
            // console.log(item);
            if(item.checked == '1'){
              this.subtotal += item.salePrice * item.productNum;//商品的总价格
